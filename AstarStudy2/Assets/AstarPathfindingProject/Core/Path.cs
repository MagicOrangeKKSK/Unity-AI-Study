...
namespace Pathfinding {
	...
	public abstract class Path : IPathInternals {
		...
		protected float fireRange;//玩家火力范围
		protected float sqrFireRange;//为了避免计算距离的时候 开方运算的性能
		protected int penaltyAmount;//额外代价
#endif
		...
		internal uint GetTraversalCost (GraphNode node) {
			...
			while (wait)
			{
				UnityMainThread.AddJob(() => 
				{
					...
					{
						if (Physics.Raycast(nodePos, distance, out RaycastHit hit))
						{
